...
class DemandAssigner:
    ...
    def is_in_cap_region_faster(
        ...
        while len(obj_id_to_demand_map):
            # Select `obj_id`
            obj_id, demand = obj_id_to_demand_map.popitem()
            demand_to_assign = min(demand, self.demand_delta)

            if demand != demand_to_assign:
                obj_id_to_demand_map[obj_id] = demand - demand_to_assign
