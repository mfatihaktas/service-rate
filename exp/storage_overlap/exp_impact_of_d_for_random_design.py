...
def plot_frac_demand_vectors_covered_vs_d(
    ...
    def plot_(
        ...
        for prob_obj_is_active in numpy.linspace(0.1, 0.5, 10):
    ...
    fontsize = 16
    plot.legend(fontsize=14, loc="upper right")  # bbox_to_anchor=(1.25, 0.65)
    ...
    plot.title(
        ...
        rf"$d= {d_max}$, "
        rf"$m= {maximal_load}$, "
        rf"$\rho \sim {demand_for_active_obj} \times $" + r"$\mathrm{Bernoulli}(p)$"
