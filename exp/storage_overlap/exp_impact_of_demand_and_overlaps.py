...
def plot_P_for_bernoulli_demand(
    k: int,
    d: int,
    active_obj_demand_rv: random_variable.Bernoulli,
    maximal_load: float,
    run_sim: bool = False,
    num_samples: int = 300,
    num_sim_run: int = 3,
    ...
    def plot_(
        ...
        for p in numpy.linspace(0.1, 0.6, 20):
            ...
            demand_vector_sampler = demand.DemandVectorSamplerWithFixedNumActiveObjs(
                ...
                num_active_objs=storage_design.k,
                active_obj_demand_rv=active_obj_demand_rv_,
            )
        ...
        if run_sim:
            plot.errorbar(p_list, E_frac_of_demand_vectors_covered_list, yerr=std_frac_of_demand_vectors_covered_list, label=f"{storage_design.repr_for_plot()}", color=color, marker=next(marker_cycle), linestyle="dotted", lw=2, mew=3, ms=5)

        plot.plot(p_list, P_ub_list, label=f"{storage_design.repr_for_plot()}, UB", color=color, marker=next(marker_cycle), linestyle="dotted", lw=2, mew=3, ms=5)
        # plot.plot(p_list, P_lb_list, label=f"{storage_design.repr_for_plot()}, LB", color=color, marker=next(marker_cycle), linestyle="dotted", lw=2, mew=3, ms=5)
